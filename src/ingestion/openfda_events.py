...
def main() -> None:
    parser = argparse.ArgumentParser(description="Pull openFDA drug event reports from FDA using the configured analysis scope.")
    parser.add_argument("--scope", type=Path, default=SCOPE_PATH, help="YAML file defining dates, drugs, and reactions.")
    parser.add_argument(
        "--validation",
        action="store_true",
        help="Also build the validation dataset from configured validation drugs.",
    )
    args = parser.parse_args()
    run_openfda_extract(args.scope, fetch_validation=args.validation)
